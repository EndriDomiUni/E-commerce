<div class="container my-2">
    <section>
        ...
        if (isset($_SESSION['search'])) {
            ...
            echo '<div>La ricerca non ha trovato nessun risultato, Vai a <a href="index.php">Home</a>! </div>';
        }
        ?>
    </section></div>
    ...
    if (isset($productId) && $productId > 0) {
        if (isset($session)) {
            if (!($session->getClaimTypeFromId($session->getCurrentUser()[CLAIM_ID]) === CLAIM_SELLER_DESC) &&
                ...
                echo '<button class="btn btn-primary ml-auto" type="submit" name="add-product-' . $productId . '-in-wishlist">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-heart-fill" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z"/>
                        </svg>
                    </button>';
                echo '<button class="btn btn-primary ml-auto" type="submit" name="add-article-' . $productId . '-in-cart">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bag-fill" viewBox="0 0 16 16">
                            <path d="M8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1zm3.5 3v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5z"/>
                        </svg>
                        Add to cart
                      </button>';
                echo '<button class="btn btn-primary ml-auto" type="submit" name="buy-now-' . $productId . '">
